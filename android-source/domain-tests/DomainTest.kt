...
private fun assertEquals(expected: Any?, actual: Any?, name: String) { check(expected == actual) { "$name: expected=$expected actual=$actual" } }
private fun assertTrue(value: Boolean, name: String) { check(value) { "$name: expected true" } }
...
fun main() {
    ...
    assertEquals(ViewerKind.RHWP, DocumentViewerStrategy.resolve("공문.hwp", null), "hwp rhwp viewer")
